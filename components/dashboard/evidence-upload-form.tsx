...
export function EvidenceUploadForm({ cases }: EvidenceUploadFormProps) {
    ...
    for (const fileWithHash of files) {
      if (fileWithHash.hash) {
        await storeEvidence({
          ...
          fileName: fileWithHash.file.name,
          fileType: fileWithHash.file.type,
          fileHash: fileWithHash.hash,
          sourcePlatform: formData.sourcePlatform,
          dateOfIncident: new Date(formData.dateOfIncident),
          description: formData.description,
          category: formData.category,
          fileSize: fileWithHash.file.size,
        })
      }
    }
          ...
          <CardContent className="p-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button asChild className="bg-primary hover:bg-primary/90 text-primary-foreground">
                <Link href={formData.caseId ? `/dashboard/cases/${formData.caseId}` : "/dashboard/evidence"}>
                  {formData.caseId ? "View Case" : "View Evidence Locker"}
                </Link>
              </Button>
              <Button
                variant="outline"
